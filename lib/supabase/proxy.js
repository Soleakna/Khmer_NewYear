...
export async function updateSession(request) {
  ...
  //
  // The guide also checks `data?.claims` here and redirects signed-out users
  // to /login. This archive has no auth routes yet (they arrive in a later
  // sprint), so we intentionally skip that branch: every visitor may browse
  // the public pages. Add the /login branch here when auth exists.
